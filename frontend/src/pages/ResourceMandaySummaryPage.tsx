...
import { PROJECT_STATUSES, PROJECT_TYPES, type Project, type ResourceMandaySummaryRow } from '../api/types';
import { useAuth } from '../auth/AuthContext';
import { PivotSummaryTable, type PivotRow } from '../components/PivotSummaryTable';
import './MandaySummaryPage.scss';

// Status/type → chip colour class (shared badge modifiers). Internal has no class → neutral grey dot.
...
export default function ResourceMandaySummaryPage() {
  ...
  // Chip counts come from the full project population, unaffected by chip selection (mirrors the Projects list).
  useEffect(() => {
  ...
  // (Re)load the pivot whenever the filter changes; keep the current table visible while refetching.
  useEffect(() => {
    ...
    api
      .get<ResourceMandaySummaryRow[]>(`/resource-manday-summary${qs ? `?${qs}` : ''}`)
