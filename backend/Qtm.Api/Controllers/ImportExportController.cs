...
public class ImportExportController(QtmDbContext db, ExcelService excel) : ControllerBase
{
    ...
    private static readonly string[] ValidProjectTypes = ["Implement", "Customize", "Training", "Internal", "MA", "Other"];
    ...
    public async Task<ActionResult<ImportResult>> ImportProjects(IFormFile file)
    {
        ...
        foreach (var row in rows)
        {
            ...
            {
                errors.Add($"แถว {line}: Type ต้องเป็น Implement/Customize/Training/Internal/MA/Other (พบ '{row.Type}')");
                skipped++;
                continue;
            }
