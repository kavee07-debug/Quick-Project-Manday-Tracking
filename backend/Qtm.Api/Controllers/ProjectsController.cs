...
{
    private static readonly string[] ValidTypes = ["Implement", "Customize", "Training", "Internal", "MA", "Other"];
    ...
    private static ActionResult? ValidateChoices(ProjectUpsert req, ControllerBase c)
    {
        if (!string.IsNullOrWhiteSpace(req.Type) && !ValidTypes.Contains(req.Type))
            return c.BadRequest(new { message = "Type ต้องเป็น Implement/Customize/Training/Internal/MA/Other" });
